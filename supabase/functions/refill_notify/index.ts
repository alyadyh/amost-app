...
async function sendNotification(notif: Notification, pushToken: string) {
  const { med_name, remaining_duration } = notif;
  ...
  const res = await fetch("https://exp.host/--/api/v2/push/send", {
    ...
    body: JSON.stringify({
      to: pushToken,
      sound: "default",
      title: `${med_name} hampir habis!`,
      body:
        `Durasi pengobatanmu tersisa ${remaining_duration} hari lagi, lakukan refill segera.`,
      data: {
        notifId: notif.id,
        medId: notif.medicine_id,
