...
const ShareReport = ({ userName }: { userName: string }) => {
  ...
  const filteredLogs = dummyLogs.filter(log => {
    return log.taken === true && isAfter(parseISO(log.log_date), oneMonthAgo)
  })

  // Group logs by date
  const groupedLogs = filteredLogs.reduce<{ [key: string]: { med_name: string, time: string }[] }>((acc, log) => {
    const log_date = log.log_date
    const med = dummyMeds.find(med => med.id === log.id)
    if (med) {
      if (!acc[log_date]) acc[log_date] = []
      if (log.log_time) {
        acc[log_date].push({ med_name: med.med_name, time: log.log_time })
      }
    }
    return acc
  }, {})
