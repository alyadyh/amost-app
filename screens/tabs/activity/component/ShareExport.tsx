...
const ShareReport = ({ userName }: { userName: string }) => {
  ...
  // Filter logs taken within the last month
  const filteredLogs = dummyLogs.filter(log => {
    return log.taken === true && isAfter(parseISO(log.date), oneMonthAgo)
  })

  // Group logs by date
  const groupedLogs = filteredLogs.reduce<{ [key: string]: { medName: string, time: string }[] }>((acc, log) => {
    const date = log.date
    const med = dummyMeds.find(med => med.id === log.id)
    if (med) {
      if (!acc[date]) acc[date] = []
      if (log.time) {
        acc[date].push({ medName: med.medName, time: log.time })
      }
    }
    return acc
  }, {})
  ...
  const sortedGroupedLogs: [string, { medName: string, time: string }[]][] = Object.entries(groupedLogs).sort(([dateA], [dateB]) => parseISO(dateB).getTime() - parseISO(dateA).getTime());
  ...
  const generateHTML = () => {
    ...
    sortedGroupedLogs.forEach(([date, logs]) => {
        ...
        <tr>
            ...
            <td style="padding: 8px 12px">${logs[0].medName}</td>
            <td style="padding: 8px 12px">${logs[0].time}</td>
        </tr>`

        logs.slice(1).forEach((log: { medName: string, time: string }) => {
        logTableRows += `
            <tr>
            <td style="padding: 8px 12px">${log.medName}</td>
